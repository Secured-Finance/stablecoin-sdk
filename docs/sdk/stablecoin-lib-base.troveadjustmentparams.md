...
[Home](./index.md) &gt; [@secured-finance/stablecoin-lib-base](./stablecoin-lib-base.md) &gt; [TroveAdjustmentParams](./stablecoin-lib-base.troveadjustmentparams.md)
...
Defining both `depositCollateral` and `withdrawCollateral`<!-- -->, or both `borrowDebtToken` and `repayDebtToken` at the same time is disallowed, and will result in a type-checking error.
...
<tr> <td> borrowDebtToken? </td> <td> T </td> <td> <i>(Optional)</i> The amount of DebtToken that's borrowed. </td> </tr>

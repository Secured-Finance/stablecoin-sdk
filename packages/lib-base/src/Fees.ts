 ...
 *
 * @remarks
 * Returned by the {@link ReadableProtocol.getFees | getFees()} function.
 *
 * @public
 */
export class Fees {
   ...
   *
   * @param redeemedFractionOfSupply - The amount of DebtToken being redeemed divided by the total supply.
   ...
   * precise, it depends on the fraction of the redeemed amount compared to the total DebtToken supply,
   * which must be passed as a parameter.
   *
   * To calculate the redemption fee in DebtToken, multiply the redeemed DebtToken amount with the redemption
   * rate.
   *
   * @example
   * ```typescript

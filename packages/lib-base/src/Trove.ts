...
/** @internal */ export type _DebtTokenBorrowing<T> = { borrowDebtToken: T };
/** @internal */ export type _DebtTokenRepayment<T> = { repayDebtToken: T };
...
/** @internal */ export type _NoDebtTokenBorrowing = Partial<_DebtTokenBorrowing<undefined>>;
/** @internal */ export type _NoDebtTokenRepayment = Partial<_DebtTokenRepayment<undefined>>;
...
export type _DebtChange<T> =
  | (_DebtTokenBorrowing<T> & _NoDebtTokenRepayment)
  | (_DebtTokenRepayment<T> & _NoDebtTokenBorrowing);

/** @internal */
export type _NoDebtChange = _NoDebtTokenBorrowing & _NoDebtTokenRepayment;

/**
 * Parameters of an {@link TransactableProtocol.openTrove | openTrove()} transaction.
 *
 * @remarks
 ...
 *
 *   <tr>
 *     <td> borrowDebtToken </td>
 *     <td> T </td>
 *     <td> The amount of DebtToken that's borrowed. </td>
 *   </tr>
 *
 * </table>
 *
...
export type TroveCreationParams<T = unknown> = _CollateralDeposit<T> &
  ...
  _DebtTokenBorrowing<T> &
  _NoDebtTokenRepayment;

/**
 * Parameters of a {@link TransactableProtocol.closeTrove | closeTrove()} transaction.
 *
 * @remarks
 ...
 *
 *   <tr>
 *     <td> repayDebtToken? </td>
 *     <td> T </td>
 *     <td> <i>(Optional)</i> The amount of DebtToken that's repaid. </td>
 *   </tr>
 *
 * </table>
 *
...
export type TroveClosureParams<T> = _CollateralWithdrawal<T> &
  ...
  Partial<_DebtTokenRepayment<T>> &
  _NoDebtTokenBorrowing;

/**
 * Parameters of an {@link TransactableProtocol.adjustTrove | adjustTrove()} transaction.
 *
 * @remarks
 ...
 * least one.
 *
 * Defining both `depositCollateral` and `withdrawCollateral`, or both `borrowDebtToken` and `repayDebtToken`
 ...
 *     <td> borrowDebtToken? </td>
 *     <td> T </td>
 *     <td> <i>(Optional)</i> The amount of DebtToken that's borrowed. </td>
 *   </tr>
 *
 *   <tr>
 *     <td> repayDebtToken? </td>
...
const debtChangeFrom = <T>({
  ...
  if (borrowDebtToken !== undefined && repayDebtToken !== undefined) {
    throw new Error(
      "TroveAdjustmentParams: 'borrowDebtToken' and 'repayDebtToken' can't be present at the same time"
    );
  }

  if (borrowDebtToken !== undefined) {
    return { borrowDebtToken };
  }

  if (repayDebtToken !== undefined) {
    return { repayDebtToken };
  }
};
...
export class Trove {
   ...
   */
  get netDebt(): Decimal {
    if (this.debt.lt(LIQUIDATION_RESERVE)) {
      throw new Error(`netDebt should not be used when debt < ${LIQUIDATION_RESERVE}`);
    }

    return this.debt.sub(LIQUIDATION_RESERVE);
  }
   ...
   *
   * @example
   * Can be used to check whether the protocol is in recovery mode by using it on the return
   * value of {@link ReadableProtocol.getTotal | getTotal()}. For example:
   *
   * ```typescript
  ...
  private _debtChange({ debt }: Trove, borrowingRate: Decimalish): _DebtChange<Decimal> {
    return debt.gt(this.debt)
      ? { borrowDebtToken: unapplyFee(borrowingRate, debt.sub(this.debt)) }
      : { repayDebtToken: this.debt.sub(debt) };
  }
    ...
    if (that.isEmpty) {
      return troveClosure(
        this.netDebt.nonZero
          ? { withdrawCollateral: this.collateral, repayDebtToken: this.netDebt }
  ...
  apply(
    ...
    switch (change.type) {
      ...
      case "creation": {
        ...
        const { depositCollateral, borrowDebtToken } = change.params;

        return new Trove(
          depositCollateral,
          LIQUIDATION_RESERVE.add(applyFee(borrowingRate, borrowDebtToken))
        );
      }
      ...
      case "adjustment": {
        const {
          setToZero,
          params: { depositCollateral, withdrawCollateral, borrowDebtToken, repayDebtToken }
        } = change;

        const collateralDecrease = withdrawCollateral ?? Decimal.ZERO;
        const collateralIncrease = depositCollateral ?? Decimal.ZERO;
        const debtDecrease = repayDebtToken ?? Decimal.ZERO;
        const debtIncrease = borrowDebtToken
          ? applyFee(borrowingRate, borrowDebtToken)
          : Decimal.ZERO;
  ...
  /**
   * Calculate the result of an {@link TransactableProtocol.openTrove | openTrove()} transaction.
   *
  ...
  /**
   * Calculate the parameters of an {@link TransactableProtocol.openTrove | openTrove()} transaction
  ...
  /**
   * Calculate the result of an {@link TransactableProtocol.adjustTrove | adjustTrove()} transaction
   * on this Trove.
   *
   * @param params - Parameters of the transaction.
  ...
  /**
   * Calculate the parameters of an {@link TransactableProtocol.adjustTrove | adjustTrove()}
 ...
 * example to represent the {@link ReadableProtocol.getTotal | total collateral and debt} locked up
 * in the protocol.
 *

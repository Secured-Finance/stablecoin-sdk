...
import { LIQUIDATION_RESERVE, MAXIMUM_BORROWING_RATE, MINIMUM_DEBT } from "../src/constants";

import { Decimal, Difference } from "../src/Decimal";
import { Trove, _emptyTrove } from "../src/Trove";

const liquidationReserve = Number(LIQUIDATION_RESERVE);
const maximumBorrowingRate = Number(MAXIMUM_BORROWING_RATE);

const maxDebt = 10 * Number(MINIMUM_DEBT);
        ...
        fc.property(validNonEmptyTrove(), validNonEmptyTrove(), (a, b) => {
          ...
          return (
            ...
            (change.type === "adjustment" &&
              ...
              !change.params.borrowDebtToken?.isZero &&
              !change.params.repayDebtToken?.isZero)
          );
        })
      );
    });
      ...
      fc.assert(
        fc.property(fc.float({ max: maximumBorrowingRate }), borrowingRate => {
          const withMinimumDebt = Trove.recreate(
            new Trove(Decimal.ONE, MINIMUM_DEBT),
            borrowingRate
          );

          const ret = Trove.create(withMinimumDebt, borrowingRate).debt.gte(MINIMUM_DEBT);
